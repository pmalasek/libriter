...
export interface Chapter {
  ...
  duration_seconds: number
  /**
   * Velikost audio souboru v bajtech. `0` znamená neznámou (záznam z doby
   * před migrací 012) – mobil pak místo odhadu použije `Content-Length`
   * ze samotného stahování.
   */
  size_bytes: number
}
...
export interface SessionPositionRequest {
  ...
  /**
   * Čas vzniku pozice na klientovi (ISO 8601). Web ho neposílá a server
   * dosadí své „teď“; mobil ho vyplňuje, aby offline dávka nepřepsala
   * novější pozici z jiného zařízení.
   */
  recorded_at?: string
  device_id?: string
}
...
/**
 * Jedna položka dávky POST /sessions/sync – totéž co zápis pozice, jen
 * s vlastním ID (podle něj se pozná opakovaná dávka) a se session v těle.
 */
export interface SyncEvent {
  /** UUID přidělené klientem. */
  id: string
  session_id: string
  book_id: string
  chapter_id?: string
...
/** POST /sessions/sync – nejvýš SYNC_BATCH_LIMIT událostí na dávku. */
 ...
 *  - `stale`     – poslech se připsal, pozici drží novější zápis odjinud
 *  - `duplicate` – server tuhle událost už jednou započetl
 *  - `rejected`  – neprošla kontrolou; `error` říká proč
 */
export type SyncStatus = 'applied' | 'stale' | 'duplicate' | 'rejected'
...
export interface SyncResponse {
  results: SyncResult[]
  /** Aktuální stav poslechů, kterých se dávka dotkla. */
  sessions: PlaySession[]
}

/** Strop serveru na jednu dávku (service.MaxSyncEvents). */
...
/**
 * POST /auth/mobile-token – dlouhodobý token mobilní aplikace (výchozí rok).
 * Otevírá API stejně jako přihlašovací token, ale ne audio; další mobilní
 * token jím vyrazit nejde, obnovuje se skutečným přihlášením.
 */
export interface MobileTokenRequest {
  /** Jen do serverového logu; nikde se neukládá. */
  device_name?: string
}

export type MobileToken = StreamToken
